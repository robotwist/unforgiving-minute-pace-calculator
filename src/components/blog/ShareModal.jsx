...
const ShareModal = ({ isOpen, onClose, article, url }) => {
  ...
  const shareUrls = {
    ...
    reddit: `https://reddit.com/submit?url=${encodeURIComponent(shareUrl)}&title=${encodeURIComponent(article.title)}`,
    ...
    <div className={styles.shareModalOverlay} onClick={onClose}>
      <div className={styles.shareModalContent} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.copyLinkSection}>
          <div className={styles.copyLinkTitle}>Or copy link</div>
          <input
            type="text"
            value={shareUrl}
            readOnly
        ...
        <div className={styles.shareStats}>
          <span>💡 Help others discover great running content</span>
        </div>
      </div>
    </div>
  );
